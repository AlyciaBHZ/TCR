...
class ScaffoldRetrievalDataset(Dataset):
    ...
    def _load(self) -> List[Dict[str, str]]:
        ...
        valid = [s for s in samples if s["peptide"] and s["mhc"] and s["h_v_seq"] and s["h_j_seq"]]
        print(f"[Dataset] Filtered {len(samples)} -> {len(valid)} (require peptide/mhc/hv_seq/hj_seq)")
        return valid
    ...
    def _compute_pos_weight(self) -> Dict[str, torch.Tensor]:
        """Compute inverse-frequency pos_weight per gene type."""
        ...
        for g, cnt in counts.items():
            ...
            for i in range(len(self.gene_vocab[g])):
                c = cnt.get(i, 0)
                # inverse frequency scaling; avoid div by zero
                w = total / (c + 1e-3)
                weights.append(w)
...
def collate_fn_factory(
    ...
    def collate(batch: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        allele_ids, mhc_groups, pmhc_groups = [], [], []
        ...
        for sample in batch:
            ...
            # Multi-hot targets (MHC group)
            tgt_mhc = dataset.group_targets_mhc.get(sample["mhc"], {"h_v": [], "h_j": [], "l_v": [], "l_j": []})
            tgt_pmhc = dataset.group_targets_pmhc.get((sample["peptide"], sample["mhc"]), tgt_mhc)
            multihot_hv.append(torch.tensor(tgt_mhc["h_v"], dtype=torch.float))
            multihot_hj.append(torch.tensor(tgt_mhc["h_j"], dtype=torch.float))
            multihot_lv.append(torch.tensor(tgt_mhc["l_v"], dtype=torch.float))
            multihot_lj.append(torch.tensor(tgt_mhc["l_j"], dtype=torch.float))
